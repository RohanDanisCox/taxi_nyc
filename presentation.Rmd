...
shap_fare <- read_feather("shap_fare.feather")
shap_time <- read_feather("shap_time.feather")
nested_train_coord <- read_feather("nested_train_coord.feather")
nested_train_zone <- read_feather("nested_train_zone.feather")
...
company_plan <- read_feather("company_plan.feather")
```
...
```{r eval = TRUE, echo = FALSE}
  ggplot(train, aes(passenger_count)) +
    ...
    labs(x = "Number of Passengers", y = "Frequency", subtitle = "Resembles a poisson distribution") +
    theme_minimal()
```
...
```{r eval = TRUE, echo = FALSE}
  ggplot(train, aes(payment_type)) +
      ...
      labs(x = "Payment Method", y = "Frequency", subtitle = "Bimodal categorical variable") +
      theme_minimal()
```
...
```{r eval = TRUE, echo = FALSE}
  ggplot(train, aes(fare_amount)) +
      ...
      labs(x = "Fare Amount", y = "Frequency", subtitle = "Notice the strange tick at $52") +
      theme_minimal()
```
...
```{r eval = TRUE, echo = FALSE}
  ggplot(train,aes(tip_amount, fill = payment_type)) +
    ...
    labs(x = "Tip Amount", y = "Frequency", fill = "Payment Type") +
    theme_minimal()
```

The distribution of tip amount by card, where we can more reliably consider the tip amount, resembles a poisson distribution around a mean of `r train %>% select(tip_amount,payment_type) %>% filter(payment_type == "CRD")%>% summarise(mean = mean(tip_amount)) %>% pull() %>% round(2)`.
...
ggplot(card_tip, aes(tip_amount)) +
    ...
    labs(x = "Tip Amount", y ="Frequency", subtitle = "Resembles a poisson distribution") +
    theme_minimal()
```
...
```{r eval = TRUE, echo = FALSE}
  ggplot(busy_hours,aes(x = pickup_hour,y= count, fill = top_5)) +
    ...
    scale_y_continuous(labels = comma) +
    ggtitle("Top 5 Busiest Hours") +
    labs(fill = "Top 5", x = "Pickup Hour", y = "Frequency", subtitle = "6-11pm is the busiest period for taxi journeys") + 
    theme_minimal()
```
...
```{r eval = TRUE, echo = FALSE, warning= FALSE}
  ...
  distinct_sample <- distinct_pickup[sample_index,]
    ...
    ggplot(distinct_sample, aes(x = pickup_longitude, y = pickup_latitude)) + 
      ...
      labs(x = "Longitude", y = "Latitude", subtitle = "Easy to make out Manhattan and even Central Park") + 
      theme_minimal()
```
...
high_sd_map <- taxi_zone_busy %>%
  ...
  ggplot(high_sd_map) + 
      ...
      labs(x = "Longitude", y = "Latitude", fill = "Trip", subtitle = "We expect to see large distances") + 
...
```{r eval = TRUE, echo = FALSE}
  ...
  ggplot(low_sd_map) + 
      ...
      labs(x = "Longitude", y = "Latitude", fill = "Trip", subtitle = "We expect to see short distances") + 
...
```{r eval = TRUE, echo = FALSE}
  ...
  ggplot(example_1,aes(fare_amount)) +
    ...
    labs(x = "Fare Amount", y = "Frequency", subtitle = "Looks fairly symmetrical") +
    theme_minimal()
```
...
```{r eval = TRUE, echo = FALSE}
  ...
  ggplot(example_2,aes(fare_amount)) +
    ...
    labs(x = "Fare Amount", y = "Frequency", subtitle = "Right skew outliers - not symmetrical") +
    theme_minimal()
```
...
```{r eval = TRUE, echo = FALSE}
  example_3 <- trips %>%
    ...
    ggplot(example_3,aes(trip_time_in_secs)) +
      ...
      labs(x = "Fare Amount", y = "Frequency", subtitle = "Normal and hence symmetrical distribution") +
      theme_minimal()
```
...
```{r eval = TRUE, echo = FALSE}
  ggplot(val_pred_rf,aes(x = fare_amount,pred)) + 
      ...
      labs(x = "Fare Amount", y = "Predicted Fare Amount", subtitle = "Seems to predict the data reasonably well") +
      theme_minimal()
```
...
```{r eval = TRUE, echo = FALSE}
  ggplot(val_pred_rf_2,aes(x = tip_amount,pred)) + 
      ...
      labs(x = "Tip Amount", y = "Predicted Fare Amount", subtitle = "Prediction underestimates tips") +
      theme_minimal()
```
...
```{r eval = TRUE,echo = FALSE}
  ggplot(nested_train_coord,aes(x = pickup_hour,y = r.squared, colour = day_of_week)) +
    ...
    labs(x = "Hour", y = "R Squared", colour = "Day of Week", subtitle = "All models explain less than 50% of the variance in fare amount") + 
...
```{r eval = TRUE,echo = FALSE}
  ggplot(nested_train_zone,aes(x = pickup_hour,y = r.squared, colour = day_of_week)) +
    ...
    labs(x = "Hour", y = "R Squared", colour = "Day of Week", subtitle = "Using zones instead of coordinates drastically improves the models") + 
    scale_color_discrete() +
    theme_minimal()
```
...
```{r eval = TRUE, echo = FALSE}
  ...
  ggplot(top_hack_1) + 
    ...
    labs(x = "Longitude", y = "Latitude", fill = "Number of Pickups", subtitle = "Nearly exclusively La Guardia") + 
    xlim(-74.05,-73.75) + 
    ylim(40.6,40.9) + 
    theme(panel.grid.major = element_line(color = gray(0.5), linetype = "dashed", size = 0.1))
  ...
  ggplot(top_hack_2) + 
    ...
    labs(x = "Longitude", y = "Latitude", fill = "Number of Dropoffs", subtitle = "Dropping off in Manhattan") + 
    xlim(-74.05,-73.75) + 
    ylim(40.6,40.9) + 
    theme(panel.grid.major = element_line(color = gray(0.5), linetype = "dashed", size = 0.1)) 
...
```{r eval = TRUE, echo = FALSE}
  ...
  ggplot(top_10_hacks_1) + 
    ...
    labs(x = "Longitude", y = "Latitude", fill = "Number of Pickups", subtitle = "JFK over La Guardia") + 
    theme(panel.grid.major = element_line(color = gray(0.5), linetype = "dashed", size = 0.1))
  ...
  ggplot(top_10_hacks_2) + 
    ...
    labs(x = "Longitude", y = "Latitude", fill = "Number of Dropoffs", subtitle = "Trying to get back to JFK if possible") + 
    theme(panel.grid.major = element_line(color = gray(0.5), linetype = "dashed", size = 0.1)) 
...
```{r eval = TRUE, echo = FALSE}
  ...
  ggplot(company_guide) + 
    ...
    xlim(-74.05,-73.85) + 
    ylim(40.65,40.85) + 
    ggtitle("Operate for High Hours Focusing on these locations") +
